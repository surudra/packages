/*  ***************************************************************************
    ...
    * Process Name:         Create Custom Views for Manufacturing BOM outbound in Application Schema  
    * Program Name:         wm_from_mfgbom_vw.sql
    * Version #:            1.0
    ...
    * Description:              
    *
    *           Create Views in APPS schema for Manufacturing BOM Outbound
    *
    * Tables usage:     
    *           
    *
...
*/
 ...
 prompt Program : wm_from_mfgbom_vw.sql
...
/*----------------------------------------------------------------------*/
--      Create View WM_BOM_BILL_OF_MTLS_VW 
/*----------------------------------------------------------------------*/

CREATE OR REPLACE VIEW WM_BOM_BILL_OF_MTLS_VW
AS
SELECT
WMTC.WEB_TRANSACTION_ID WEB_TRANSACTION_ID,
WMTC.TRANSACTION_TYPE DOCUMENT_TYPE,
DECODE(WMTC.TRANSACTION_STATUS,0,'UPDATE',1,'INSERT') DOCUMENT_STATUS,
...
BOM.COMMON_ALT_BOM_DESIGNATOR COMMON_ALT_BOM_DESIGNATOR, -- extra column added
...
--NULL TRANSACTION_TYPE, -- Removed from 10.7 SC
MUM.UNIT_OF_MEASURE UNIT_OF_MEASURE, -- Renamed alias
NULL REVISION 
FROM
...
AND WMTC.TRANSACTION_TYPE = 'MFGBOM' 
UNION
...
TO_DATE(NULL),
NULL,
-- NULL, -- Removed for 10.7 SC
NULL
FROM WM_TRACKCHANGES WT, MTL_SYSTEM_ITEMS_KFV MSI, HR_ALL_ORGANIZATION_UNITS HOU, WM_TRACK_CHANGES_VW WTC
...
AND WTC.TRANSACTION_TYPE='MFGBOM'
AND WT.TRANSACTION_STATUS=2
AND WT.TRANSACTION_STATUS=WTC.TRANSACTION_STATUS
/

/*----------------------------------------------------------------------*/
--      Create View WM_BOM_ITEM_REVISIONS_VW 
/*----------------------------------------------------------------------*/

CREATE OR REPLACE VIEW WM_BOM_ITEM_REVISIONS_VW 
AS
SELECT 
BOM.BILL_SEQUENCE_ID BILL_SEQUENCE_ID,
MSI.CONCATENATED_SEGMENTS ITEM_NUMBER,
...
/*----------------------------------------------------------------------*/
--      Create View WM_BOM_INVENTORY_COMPS_VW 
/*----------------------------------------------------------------------*/

CREATE OR REPLACE VIEW WM_BOM_INVENTORY_COMPS_VW  
AS
SELECT
...
DECODE(BIC.INCLUDE_ON_BILL_DOCS,1,'Yes',2,'No',NULL) INCLUDE_ON_BILL_DOCS, -- Added for 10.7 SC
...
--NULL NEW_OPERATION_SEQ_NUM, -- Removed for 10.7 SC
--NULL NEW_EFFECTIVITY_DATE,  -- Removed for 10.7 SC
ML2.MEANING BOM_ITEM_TYPE,
BIC.OPERATION_LEAD_TIME_PERCENT OPERATION_LEAD_TIME_PERCENT, 
-- BOM.ASSEMBLY_TYPE ASSEMBLY_TYPE, -- Removed for 10.7 SC
-- DECODE(BOM.ASSEMBLY_TYPE,1,'BILL',2,'ECO',NULL) INTERFACE_ENTITY_TYPE, -- Removed for 10.7 SC
MUM.UNIT_OF_MEASURE UNIT_OF_MEASURE -- Renamed Alias for 10.7 SC
-- NULL TRANSACTION_TYPE -- Removed for 10.7 SC
FROM 
...
MTL_ITEM_REVISIONS MIR,
--BOM_INVENTORY_COMPONENTS BIC1,-- Removed for 10.7 SC
MFG_LOOKUPS ML1 , 
MFG_LOOKUPS ML2 , 
MFG_LOOKUPS ML3 , 
MTL_UNITS_OF_MEASURE MUM,
...
--AND BIC.OLD_COMPONENT_SEQUENCE_ID = BIC1.COMPONENT_SEQUENCE_ID(+) -- Removed for 10.7 SC
...
/*----------------------------------------------------------------------*/
--      Create View WM_BOM_SUBSTITUTE_COMPS_VW 
/*----------------------------------------------------------------------*/

CREATE OR REPLACE VIEW WM_BOM_SUBSTITUTE_COMPS_VW 
AS
SELECT
...
-- NULL NEW_SUB_COMP_NUMBER, -- Removed for 10.7 SC
-- DECODE(BOM.ASSEMBLY_TYPE,1,'BILL',2,'ECO',NULL) INTERFACE_ENTITY_TYPE, -- Removed for 10.7 SC
MUM.UNIT_OF_MEASURE UNIT_OF_MEASURE -- Renamed Alias for 10.7 SC
-- NULL TRANSACTION_TYPE -- Removed for 10.7 SC
FROM 
...
/*----------------------------------------------------------------------*/
--      Create View WM_BOM_REFERENCE_DESGS_VW 
/*----------------------------------------------------------------------*/

CREATE OR REPLACE VIEW WM_BOM_REFERENCE_DESGS_VW 
AS
SELECT
...
ML.MEANING ACD_TYPE  
-- NULL NEW_DESIGNATOR, -- Removed for 10.7 SC
-- DECODE(BOM.ASSEMBLY_TYPE,1,'BILL',2,'ECO',NULL) INTERFACE_ENTITY_TYPE, -- Removed for 10.7 SC
-- NULL TRANSACTION_TYPE -- Removed for 10.7 SC
FROM 
BOM_BILL_OF_MATERIALS BOM , 
...
/*----------------------------------------------------------------------*/
--      Create View WM_BOM_BILL_OF_MTLS_QRY_VW 
/*----------------------------------------------------------------------*/

CREATE OR REPLACE VIEW WM_BOM_BILL_OF_MTLS_QRY_VW 
AS
SELECT 
TO_NUMBER(NULL) WEB_TRANSACTION_ID,
'MFGBOM' DOCUMENT_TYPE,
