...
BEGIN
	...
	IF INSERTING THEN
		...
		-- check from PO_ACTION_HISTORY table if the PO is getting APPROVED the first time.
		-- If Yes, this PO is a New Outbound PO, else this is a revised Updated PO.

		SELECT DECODE( COUNT(*), 1, DECODE(NVL(:new.print_count,0), 0, 1, 0), 0) INTO p_rec_wm_trackChange.transaction_status
		FROM PO_ACTION_HISTORY
		WHERE OBJECT_TYPE_CODE = 'PO'
		AND OBJECT_ID = :new.po_header_id
		AND ACTION_CODE = 'APPROVE';
     	 	...
     	 	p_rec_wm_trackChange.transaction_id:=:new.po_header_id;
     	 	p_rec_wm_trackChange.date_created:=SYSDATE;
     	 	p_rec_wm_trackChange.comments:='PURCHASE ORDER '||:new.segment1||' UPDATED AND APPROVED.';
	 	p_rec_wm_trackChange.processed_flag:='N';
