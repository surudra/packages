...
CREATE OR REPLACE PACKAGE Wm_Inv_Item_Imp_Handler_Pkg AUTHID CURRENT_USER AS
...
PROCEDURE WM_HANDLE_INV_ITEM(p_user_responsibility IN VARCHAR,
			    ...
			    o_message OUT VARCHAR,
			    v_errmsg OUT VARCHAR,
			    p_create_update IN NUMBER, -- 1-> Create 2-> Update To be populated at runtime
			    p_process_set_id IN NUMBER -- Passed from webmethods generate separate values for Create and Update transactions
			    )
IS
...
BEGIN
	  ...
	  wm_conc_request_pkg.wm_request_submit(v_application_id,
						 ...
						 to_char(p_create_update) -- Create or Update Flag
           			  	     	 );
