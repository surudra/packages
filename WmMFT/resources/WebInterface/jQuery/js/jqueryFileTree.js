...
localizations.FilterCustomText = "(Items : {totalItems} , Folders: {folders} Files: {files})";
...
//Webinterface labels
localizations.CopyrightText = "&copy; 2014 <a target=\"_blank\" href=\"http://www.softwareag.com/\">CrushFTP</a>";
			...
			$(this).each(function () {
					...
					getCustomForm(customFormName, function (data, hasForm, formName, showAlways) {
						if (hasForm) {
							...
							function displayError(msg, isSuccess) {
                                ...
                                if(isSuccess)
                                {
                                    form.before("<div style='float: none;border: solid 1px #CCC;-moz-box-shadow: 2px 1px 3px #CCC;-webkit-box-shadow: 2px 1px 3px #CCC;box-shadow: 2px 1px 3px #CCC;background-color: #FEFFD3;padding: 18px;'><h3 style='background-image: url(/WebInterface/jQuery/images/checkmark.png);background-position: left 1px;background-repeat: no-repeat;padding-left:20px;'>"+localizations.RegisterWindowProcessCompletedTitle+"</h3><br> <div style='padding-left:20px;'> "+localizations.RegisterWindowProcessCompleteMessage+" <br><br><br> <a style='color:black;' href='javascript:window.location.reload();'>Register Another User</a> | <a  style='color:black;' href='/WebInterface/login.html'>Login</a></div></div>");
                                }
                                else
                                {
								    form.before("<div class='attention' style='float: none;border: solid 1px #CCC;-moz-box-shadow: 2px 1px 3px #CCC;-webkit-box-shadow: 2px 1px 3px #CCC;box-shadow: 2px 1px 3px #CCC;background-color: #FEFFD3;padding: 10px;'><h3 style='padding-left:20px;'>"+msg+" : </h3><br> <div style='padding-left:20px;'>"+localizations.RegisterWindowProcessFailedMessage+"</div></div>");
                                }
                ...
                function refreshTreePaths(paths)
                {
                    ...
                    for (var i = 0; i < paths.length; i++) {
                        ...
                        if(rel != rootPath)
                        {
                            var prnt = filesContainer.find("a[rel='"+rel+"']").closest("td").removeClass('expanded').addClass('collapsed');
                            var newItems = [];
                    ...
                    function refreshNow(){
                        ...
                        {
                            var rel = btns[0];
                            var curBtn = $(filesContainer.find("a[rel='"+rel+"']").closest("td").removeClass('expanded').addClass('collapsed').closest('tr').find("span.expandButton"));
                            if(curBtn.length>0)
                            {
                ...
                function expandAllItems(expand)
                {
                    var filesContainer = $("#filesContainer");
                    ...
                    {
                        var toExpand = [];
                        filesContainer.find("td[root_dir='"+curDir+"']:not(.expanded)").find("span.expandButton").each(function(){
                    ...
                    {
                        if(filesContainer.find("td.expanded[root_dir='"+curDir+"']:first").find("span.expandButton").length>0)
                        {
                            var loadingIndicator = $("#loadingIndicator");
                            loadingIndicator.dialog("option", "modal", true);
                            loadingIndicator.dialog('open');
                            function refreshNow()
                            {
                                var toCollapse = filesContainer.find("td.expanded[root_dir='"+curDir+"']:first").find("span.expandButton");
				...
				function bindCountOfFiles() {
			        ...
			        var fVal = $("#filter").val();
              if (fVal.toLowerCase().search("script>") > -1)
							    FilterItemCountText = getLocalizationKey("FilterCustomText");
							var itemCount = FilterItemCountText.replace("{filterVal}",fVal ).replace("{totalItems}", window.listingInfo.totalItems).replace("{folders}", dirs).replace("{files}", files);
							$("#itemCount").prepend("<span style='margin-right:15px;color:#777;'>" + _escapeHTML(itemCount) + "</span>");
						}
					} else {
              ...
              if (fVal.toLowerCase().search("script>") > -1)
				...
				function bindSelectionMenu() {
                    ...
                    if(isCrush7Ready())
                    {
                        $("a.treeViewExpandAllLink,a.treeViewCollapseAllLink").unbind().click(function(event) {
                            if (currentView() != "Thumbnail") {
                                expandAllItems($(this).is(".treeViewExpandAllLink"));
                            }
                            return false;
                        });
                    }
                ...
                function bindDragDropFiles()
                {
                    if(window.isCrush7Ready())
                    {
                        ...
                        if(currentView() == "Thumbnail") {
                            ...
                            droppableItems = $(".fileThumb, .directoryThumb, .fileListWrapper", filesListing.parent().parent());
                            ...
                            droppableItems.droppable({
                              ...
                              drop : function(event, ui){
                                ...
                                if($(ui.draggable).closest('#filesBasket').length>0)
                                {
                                ...
                                else if(dropTo.is(".fileListWrapper"))
                                {
                        ...
                        else
                        {
                            ...
                            draggableItems.addClass('dndApplied');
                            draggableItems.crushDraggables("destroy");
                            draggableItems.unbind("click").click(function (evt) {
                                //evt.stopPropagation();
                                //evt.preventDefault();
                            ...
                            filesListing.find("[draggable]").dragout()
                            droppableItems = $(".dirItemTR, .fileItemTR, .fileListWrapper", filesListing.parent().parent());
                            droppableItems.droppable("destroy");
                            droppableItems.droppable({
                              accept: ".fileThumb, .directoryThumb, td",
                              addClasses: false,
                              ...
                              drop : function(event, ui){
                                ...
                                var items = draggingInfo.items;
                                var target;
                                if($(event.target).is(".fileListWrapper"))
                                {
                                    target = unescape(hashListener.getHash().toString().replace("#", ""));
                                }
                                else
					...
					if (currentView() != "Thumbnail") {
						...
						if (ok) {
							...
							$(t).find('span.expandButton').unbind(o.folderEvent).bind(o.folderEvent, function (event, callback) {
                                ...
                                var srchNotifVisible = isSearchResultPanel.is(":visible");
                                event.stopPropagation();
                                event.preventDefault();
                                $(this).trigger('mouseup');
				...
				function generateHTMLForBasketItem(opt)
				{
					...
					if (opt) {
						...
						if (currentView(true) == "Thumbnail") {
                            if (opt.type === 'DIR') {
                                var downloadURL = _isUIDNDReady ? "draggable=\"true\" data-downloadurl=\"application/octet-stream:" + fileName + ".zip:" +linkURL+'?command=downloadAsZip&zipName='+fileName+'.zip&path='+crushFTPTools.htmlEncode(opt.file, false, true) + '&paths='+crushFTPTools.htmlEncode(opt.file, false, true)+"\"" : "";
								var dirTemplate = '<li '+downloadURL+' rootdir="'+escape(opt.path)+'" class="contextMenuItem vtip fileBoxBasket directoryThumb'+hideItem+'" preview="' + opt.preview + '" sizeinbytes="'+opt.sizeB+'"><span class="fileSelectionMark error"></span><span class="fileRemoveFromBasket error"></span><div>' + '<div class="imgBox"><div class="imgWrapper">' + '<table cellspacing="0" cellpadding="0" align="center">' + '<tbody>' + '<tr>' + '<td valign="middle" align="center"><a class="imgLink" rel="' + escape(opt.file) + '" href="javascript:void(0);">' + '<img alt="' + opt.name + '" title="' + opt.name + '" style="border: 0px none;" src="' + o.fileFolder + '"></a></td>' + '</tr>' + '</tbody>' + '</table></div>' + '</div>' + '</div>' + '<div class="imgTitle wordwrap"><a href="javascript:void(0);" rel="'+escape(opt.path)+'" >' + opt.name + '</a></div>' + '</li>';
								...
								var fileTemplate = '<li '+downloadURL+' rootdir="'+escape(opt.path)+'" class="contextMenuItem vtip fileBoxBasket fileThumb'+hideItem+'" preview="' + opt.preview + '" sizeinbytes="'+opt.sizeB+'"><span class="fileSelectionMark error"></span><span class="fileRemoveFromBasket error"></span><div>' + '<div class="imgBox"><div class="imgWrapper">' + '<table cellspacing="0" cellpadding="0" align="center">' + '<tbody>' + '<tr>' + '<td valign="middle" align="center"><a class="imgLink" rel="' + escape(opt.file) + '" href="javascript:void(0);">' + '<img alt="' + opt.name + '" title="' + opt.name + '" style="border: 0px none;" src="' + o.spinerImage + '"></a></td>' + '</tr>' + '</tbody>' + '</table></div>' + '</div>' + '</div>' + '<div class="imgTitle wordwrap"><a href="javascript:void(0);" rel="'+escape(opt.path)+'" >' +  opt.name + '</a></div>' + '</li>';
							...
							if (opt.type === 'DIR') {
                                var downloadURL = _isUIDNDReady ? "draggable=\"true\" data-downloadurl=\"application/octet-stream:" + fileName + ".zip:" +linkURL+'?command=downloadAsZip&zipName='+fileName+'.zip&path='+crushFTPTools.htmlEncode(opt.file, false, true) + '&paths='+crushFTPTools.htmlEncode(opt.file, false, true)+"\"" : "";
								fileList.push('<tr '+downloadURL+' rootdir="'+escape(opt.path)+'" rel="' + escape(opt.path) + '" preview="'+ opt.preview +'" class="jqueryFileTree directoryThumb'+hideItem+'"><td><span class="fileRemoveFromBasketTree error" style="margin-top:2px;"></span></td><td class="columnSelect"><input class="chkBox" type="checkbox"/></td><td preview="' + opt.preview + '" class="contextMenuItem directory columnName wordwrap imgTitle" sizeinbytes="'+opt.sizeB+'"><a class="imgLink" rel="' + escape(opt.file) + '" href="javascript:void(0);">' + wbr(opt.name, 30) + '</a></td><td class="columnPath">'+opt.path+'</td><td class="columnSize">' + opt.size + '</td><td class="columnModified">' + opt.date + '</td><td class="columnKeywords wordwrap" title="' + keywords + '">' + crushFTPTools.xmlEncode(keywords) + '</td></tr>');
							} else if (opt.type === 'FILE') {
                                var downloadURL = _isUIDNDReady ? "draggable=\"true\" data-downloadurl=\"application/octet-stream:" + fileName + ":" +linkURL+crushFTPTools.htmlEncode(opt.file, false, true)+"\"" : "";
								fileList.push('<tr '+downloadURL+' rootdir="'+escape(opt.path)+'" rel="' + escape(opt.path) + '" preview="'+ opt.preview +'" class="jqueryFileTree'+hideItem+'"><td><span class="fileRemoveFromBasketTree error" style="margin-top:2px;"></span></td><td class="columnSelect"><input class="chkBox" type="checkbox"/></td><td preview="' + opt.preview + '" class="contextMenuItem fileTR columnName fileItem wordwrap imgTitle"sizeinbytes="'+opt.sizeB+'"><a class="imgLink" rel="' + escape(opt.file) + '" href="javascript:void(0);">' + wbr(opt.name, 30) + '</a></td><td class="columnPath">'+opt.path+'</td><td class="columnSize">' + opt.size + '</td><td class="columnModified">' + opt.date + '</td><td class="columnKeywords wordwrap" title="' + keywords + '">' + crushFTPTools.xmlEncode(keywords) + '</td></tr>');
							}
						}
					}
				...
				function stripDownEmails(email) {
					...
					for (var x = 0; x < emails.length; x++) {
						...
						if (s.indexOf("<") >= 0 && s.indexOf(">") >= 0) {
							s = s.substring(s.indexOf("<") + 1, s.indexOf(">")); //pull out the email in a pretty formatted version
						}
				...
				function bindUserCustomizationInfo(callback) {
                    ...
                    function applyUserInfoToLayout(msg)
                    {
                        ...
                        if (user_priv_options && user_priv_options.indexOf("(SITE_PASS)") >= 0) {
					...
					for (var i=0;i<customizations.length;i++) //Loop through customizations
					{
                        ...
                        case "hideFilterPanel":
                         ...
                         case "hidePageSizeSelectionLink":
                            if(custItem.value == "true")
                            {
                                $("a.pageSizeSelectionLink").parent().hide();
                            }
                            break;
						case "CHECK_COL":
				...
				function applyShareCustomizations() {
					...
					if (customizations)
                    {
                        ...
                        if(window.shareWindowEnableUsernameLookup == true)
                        {
                            ...
                            $.ajax({
                                type: "POST",
                                url: "/WebInterface/function/?command=lookup_form_field&form_name=share_form&form_element_name=shareUsernames&q=",
                                success: function (data) {
                                    var checks = [];
                                    var users = [];
                                    var usersData = [];
					...
					if (_html.length > 0) {
                        ...
                        $("a[rel='QUICKVIEW']", quickViewItems).colorbox({
                            ...
                            onComplete : function(){
                                ...
                                var _fileName = contextElem.attr("filename");
                                        ...
                                        playBtn.click(function(event) {
                                            ...
                                            function showPreview()
                                            {
                                                ...
                                                setTimeout(function(){
                                                    ...
                                                    var path = o.downloadURL + "?command=download&random="+Math.random()+"&mimeType="+mimeType + "&CrushAuth=" + $.cookie("CrushAuth")+"&path="+crushFTPTools.encodeURILocal(_fileName).replace(/\&/g, "%26");
                                                    if(ext == "flv")
                                                    {
                                                        setTimeout(function(){
                                                            content.height(550);
                                                    ...
                                                    else if(ext == "mp3")
                                                    {
                                                        ...
                                                        if(window.useHTML5PlayerWhenAvailable && !!document.createElement('audio').canPlayType && !!document.createElement('audio').canPlayType("audio/mp3"))
                                                        {
                                                            ...
                                                            mPlayer.find("a").click(function(){
                                                                ...
                                                                var _path = o.downloadURL + "?command=download&random="+Math.random()+"&mimeType="+mimeType + "&CrushAuth=" + $.cookie("CrushAuth")+"&path="+crushFTPTools.encodeURILocal(_fileName).replace(/\&/g, "%26");
                                                        ...
                                                        {
                                                            content.empty().append("<a href=\""+crushFTPTools.encodeURILocal(path)+"\" class=\"media {flashvars: { mp3: '"+crushFTPTools.encodeURILocal(path)+"'}, width:700, height:60}\">Preview : \""+ _fileName.replace(/\&/g, "&amp;") +"\"</a> ");
                                                            content.find("a.media").media({
                                                                params :{
                                                                    movie : o.FilePath + "js/player_mp3_maxi.swf",
                                                                    showstop : 1,
                                                    ...
                                                    else
                                                    {
                                                        if(window.useHTML5PlayerWhenAvailable && !!document.createElement('video').canPlayType && !!document.createElement('video').canPlayType("video/"+ext+"") && (ext == "mp4" || ext == "webm" || ext == "ogg"))
                                                        {
                                                            ...
                                                            mPlayer.append('<video tabindex="0" width="700" height="500"><source type="video/'+_ext+'" src="'+(path)+'"></video><div class="clear">Preview : "'+ _fileName +'"</div><div style="margin-top:10px;font-size:11px;">Problem with playback? <a href="javascript:void(0)">Try browser player</a>');
                                                            ...
                                                            mPlayer.find("a").click(function(){
                                                                ...
                                                                content.empty().append("<a href=\""+path+"\" class=\"media {width:700, height:500}\">Preview : \""+ _fileName +"\"</a> <div style='margin-top:10px;font-size:11px;'><a href='javascript:void(0)' class='html5player'>Switch to HTML5 Player</a></div>");
                                                                content.find("a.media").media({
                                                                    params :{
                                                                        showcontrols : '1',
                ...
                window.updateLinksOnSelectionChange = function(basket){
                    ...
                    {
                        var linkURL = unescape(window.location.href.split('#')[0]);
                        var downloadURL = "application/octet-stream:archieve.zip:" +linkURL+"?command=downloadAsZip&zipName=archieve.zip&path="+crushFTPTools.htmlEncode(pathsSelected.join(":"), false, true) + "[@1]&paths="+crushFTPTools.htmlEncode(pathsSelected.join(":")+"[@2]", false, true);
				...
				window.downloadAsZip = function (context, isFolder, isBasket) {
                    ...
                    if ((window.listingInfo.selectedEverything && window.listingInfo.isSearchResult) || (window.listingInfo.selectedEverything && window.listingInfo.filtered)) {
				...
				window.loadApplet = function(signed, callback, noNote) {
                    ...
                    var height = typeof window.InstallTrigger !== 'undefined' ? "200" : "50";
				...
				function isAppletReady(signed, callback) {
					try
					{
						...
						if (appletItem && appletItem.test() == "OK")
						{
                            ...
                            $("#javaAppletDiv").height(50).removeClass('ffAppletVerify');
							if (signed) {
				...
				function changeView(type, refresh, basket) {
					...
					else
					{
						...
						if (type == "Thumbnail") {
							...
							$("#filesContainer, .treeviewExpandOptions").show();
							$("#slider").hide();
						}
							...
							if (opt) {
								...
								if (currentView() == "Thumbnail") {
                                    if (opt.type === 'DIR') {
                                        ...
                                        {
                                            title = '<div class=\'tooltip-content\'><strong>' + getLocalizationKey('TooltipNameLabelText') + ' : </strong>' + opt.name + '<br /><strong>' + getLocalizationKey('TooltipPathLabelText') + ' : </strong>' + opt.root_dir + '<br /><strong>' + getLocalizationKey('TooltipModifiedLabelText') + ' : </strong>' + opt.dateFormatted + '<br /><strong>' + getLocalizationKey('TooltipKeywordsLabelText') + ' : </strong>' + keywords + '</div>';
                                        }
                                        else
                                        {
                                            title = '<div class=\'tooltip-content\'><strong>' + getLocalizationKey('TooltipNameLabelText') + ' : </strong>' + opt.name + '<br /><strong>' + getLocalizationKey('TooltipModifiedLabelText') + ' : </strong>' + opt.dateFormatted + '<br /><strong>' + getLocalizationKey('TooltipKeywordsLabelText') + ' : </strong>' + keywords + '</div>';
                                        }
                                        var downloadURL = _isUIDNDReady ? "draggable=\"true\" data-downloadurl=\"application/octet-stream:" + fileName + ".zip:" +linkURL+'?command=downloadAsZip&zipName='+fileName+'.zip&path='+crushFTPTools.htmlEncode(opt.href_path, false, true) + '&paths='+crushFTPTools.htmlEncode(opt.href_path, false, true)+"\"" : "";
                                        ...
                                        if(window.isSearchResult || window.alwaysShowPathColumn)
                                        {
                                            title = '<div class=\'tooltip-content\'><strong>' + getLocalizationKey('TooltipNameLabelText') + ' : </strong>' + opt.name + '<br /><strong>' + getLocalizationKey('TooltipPathLabelText') + ' : </strong>' + opt.root_dir + '<br /><strong>' + getLocalizationKey('TooltipSizeLabelText') + ' : </strong>' + opt.sizeFormatted + ' <span class=\'syncSc\'>('+opt.size+' bytes)</span><br /><strong>' + getLocalizationKey('TooltipModifiedLabelText') + ' : </strong>' + opt.dateFormatted + '<br /><strong>' + getLocalizationKey('TooltipKeywordsLabelText') + ' : </strong>' + crushFTPTools.xmlEncode(keywords) + '</div>';
                                        }
                                        else
                                        {
                                            title = '<div class=\'tooltip-content\'><strong>' + getLocalizationKey('TooltipNameLabelText') + ' : </strong>' + opt.name + '<br /><strong>' + getLocalizationKey('TooltipSizeLabelText') + ' : </strong>' + opt.sizeFormatted + ' <span class=\'syncSc\'>('+opt.size+' bytes)</span><br /><strong>' + getLocalizationKey('TooltipModifiedLabelText') + ' : </strong>' + opt.dateFormatted + '<br /><strong>' + getLocalizationKey('TooltipKeywordsLabelText') + ' : </strong>' + crushFTPTools.xmlEncode(keywords) + '</div>';
                                        }
                                    ...
                                    if (opt.type === 'DIR') {
                                        if (keywords.length >= o.MaximumLengthAllowedForKeywordsString) {
                                            keywords = keywords.substr(0, o.MaximumLengthAllowedForKeywordsString) + "...";
                                        }
                                        if(window.maskedEmptyFolder)
                                        {
                                            if(opt.size + "" == "0")
                                        ...
                                        var downloadURL = _isUIDNDReady ? "draggable=\"true\" data-downloadurl=\"application/octet-stream:" + fileName + ".zip:" +linkURL+'?command=downloadAsZip&zipName='+fileName+'.zip&path='+crushFTPTools.htmlEncode(opt.href_path, false, true) + '&paths='+crushFTPTools.htmlEncode(opt.href_path, false, true)+"\"" : "";
                                        fileList.push('<tr '+downloadURL+' index="' + x + '" name="' + opt.name + '" rootdir="' + opt.root_dir + '" modified="' + opt.modified + '" style="display:none;" privs="' + escape(opt.privs) + '" rel="' + escape(unescape(opt.root_dir)) + '" class="dirItemTR jqueryFileTree ' + subdirClass + '" sizeInBytes="' + opt.size + '" size="' + opt.sizeFormatted + '" fulldate="' + opt.modified + '" Date="' + opt.dateFormatted + '" Keywords="' + crushFTPTools.xmlEncode(keywords) + '" preview="' + opt.preview + '"><td class="columnSelect"><input class="chkBox" type="checkbox"/></td><td preview="' + opt.preview + '" ' + attrs + ' class="directory collapsed columnName '+syncedItemClass+'"><span class="expandButton" rel="' + escape(unescape(opt.href_path)) + '/"><img src="' + o.collapsedImageURL + '" /></span><a href="' + crushFTPTools.htmlEncode(opt.href_path, false, true) + '" rel="' + escape(unescape(opt.href_path)) + '/">' + fileName + '</a></td><td class="columnPath listRootDir">'+opt.root_dir+'</td><td class="columnSize" size="' + opt.size + '"></td><td class="columnModified" modified="' + opt.modified + '">' + opt.dateFormatted + '</td><td class="columnKeywords" title="' + crushFTPTools.xmlEncode(keywords) + '">' +  crushFTPTools.xmlEncode(keywords) + '</td></tr>');
                                    } else if (opt.type === 'FILE') {
                                        if (keywords.length >= o.MaximumLengthAllowedForKeywordsString) {
                                            keywords = keywords.substr(
                                            0, o.MaximumLengthAllowedForKeywordsString) + "...";
                                        ...
                                        var downloadURL = _isUIDNDReady ? "draggable=\"true\" data-downloadurl=\"application/octet-stream:" + fileName + ":" +linkURL+crushFTPTools.htmlEncode(opt.href_path, false, true)+"\"" : "";
                                        fileList.push('<tr '+downloadURL+' index="' + x + '" name="' + opt.name + '" rootdir="' + opt.root_dir + '" style="display:none;"  modified="' + opt.modified + '" privs="' + escape(opt.privs) + '" rel="' + escape(unescape(opt.root_dir)) + '" class="fileItemTR jqueryFileTree ' + subdirClass + '" sizeInBytes="' + opt.size + '" size="' + opt.sizeFormatted + '" fulldate="' + opt.modified + '" Date="' + opt.dateFormatted + '" Keywords="' + crushFTPTools.xmlEncode(keywords) + '" preview="' + opt.preview + '"><td class="columnSelect"><input class="chkBox" type="checkbox"/></td><td  preview="' + opt.preview + '" ' + attrs + ' class="fileTR columnName fileItem '+syncedItemClass+'"><a href="' + crushFTPTools.htmlEncode(opt.href_path, false, true) + '" rel="' + escape(unescape(opt.href_path)) + '">' + fileName + '</a></td><td class="columnPath listRootDir">'+opt.root_dir+'</td><td class="columnSize" fulldate="' + opt.date + '" sizeInBytes="' + opt.size + '" size="' + opt.size + '">' + opt.sizeFormatted + '</td><td class="columnModified" modified="' + opt.modified + '">' + opt.dateFormatted + '</td><td class="columnKeywords" title="' + crushFTPTools.xmlEncode(keywords) + '">' + crushFTPTools.xmlEncode(keywords) + '</td></tr>');
                                    }
                                }
							}
						}
				...
				function bindBreadcrumbs() {
					...
					for (var i = dirs.length - 1; i > 0; i--) {
						if (i < dirs.length - 2) {
							crumbs = "<li><a href='#" + escape(getLocation(i + 1)) + "'>" + _escapeHTML(dirs[i]) + "</a></li>" + crumbs;
						} else {
							crumbs = "<li>" + _escapeHTML(dirs[i]) + "</li>" + crumbs;
						}
						items += 1;
					}
					...
					crumbsPanel.html(crumbs).append("<span class='refreshButton' rel='" + hashLoc + "'><img src='" + o.refreshImageURL + "' /></span>");
