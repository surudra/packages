 ...
 */
/*
 * Modified for ActiveTransfer
 * Minified using : http://jscompress.com/
 * Source used from : http://jquery-ui.googlecode.com/svn/branches/labs/selectable/demos/selectable/default.html
 */ (function ($) {
    $.widget('ui.selectableAdvanced', $.ui.mouse, {
        options: {
            closest: true,
            filter: '> *',
            keyboard: true,
        ...
        _init: function () {
            ...
            this.element.addClass("ui-selectable ui-widget").attr('tabindex', function (i, old) { // We need to set a tabindex on the root element if none is set
                return old || 0;
            });
            ...
            this.element.bind('mousedown.selectable', function (event) {
                if (self.options.disabled || event.which == 3) return false;
                ...
                if (self._mouseSelect(event, item) === true) return true;
            }).bind('mouseup.selectable', function (event) {
                if (self._listenForMouseUp || event.which == 3) {
                    self._listenForMouseUp = 0;
                    var item = self._targetIsItem(event.target);
                    if (item && self._mouseSelect(event, item) === true) return true;
                }
                ...
                if (!self.options.keyboard || self.options.disabled || self._trigger('beforeselect', event) === false) return true;
                ...
                if (event.keyCode == $.ui.keyCode.DELETE) {
                    self._trigger('remove', event);
                    event.preventDefault();
                }
                if (event.keyCode == $.ui.keyCode.HOME || event.keyCode == $.ui.keyCode.PAGE_UP) {
                    self.first(event);
                    event.preventDefault();
                }
                if (event.keyCode == $.ui.keyCode.END || event.keyCode == $.ui.keyCode.PAGE_DOWN) {
                    self.last(event);
                    event.preventDefault();
                }
				if (event.keyCode == $.ui.keyCode.ENTER) {
					if(self.options && self.options.onEnter)
					{
				...
				{
					var character = String.fromCharCode((96 <= event.keyCode && event.keyCode <= 105)? event.keyCode-48 : event.keyCode);
					if(character && character.length>0)
					{
						self._filterText += character;
						self.filter(event, self._filterText);
					}
					delay(function(){
						self._filterText = "";
					}, 1000);
            ...
            this.helper = $(document.createElement('div')).addClass("ui-selectable-lasso");
        },
        ...
        destroy: function () {
            ...
            this.element.removeClass("ui-selectable ui-selectable-disabled ui-widget").removeData("selectable").unbind(".selectable");
            this._mouseDestroy();
        },
        ...
        _mouseDrag: function (event) {
            ...
            this.items.each(function () {
                ...
                if (!item || item.element == self.element[0]) return;
                var hit = false;
                ...
                hit ? item.startSelected ? self._removeFromSelection($(this), event) : self._addToSelection($(this), event) : !item.startSelected ? self._removeFromSelection($(this), event) : self._addToSelection($(this), event);
            });
            return false;
        },
        _mouseStop: function (event) {
            ...
            for (var i = 0; i < this._selection.length; i++) {
                ...
                for (var j = 0; j < this._previousSelection.length; j++) {
                    if (this._selection[i][0] == this._previousSelection[j][0]) wasAlreadyPartOfPreviousSelection = true;
                };
            ...
            newlySelected = $($.map(newlySelected, function (i) {
                return i[0];
            }));
            newlyDeselected = $($.map(newlyDeselected, function (i) {
                return i[0];
            }));
        ...
        _mouseSelect: function (event, item) {
            ...
            if (this._trigger('beforeselect', event) === false) return true;
        ...
        _triggerDeselection: function (event) {
            ...
            triggerItems = $($.map(triggerItems, function (i) {
                return i[0];
            }));
        ...
        _clearSelection: function (triggerEvent) {
            ...
            this._previousSelection = this._selection.slice();
            this._selection = [];
            if (triggerEvent && triggerItems.length) this._trigger('deselect', triggerEvent, this._uiHash($($.map(triggerItems, function (i) {
                return i[0];
            })), 'removed'));
        },
        ...
        _addToSelection: function (item, triggerEvent) {
            if (!item.data('selectable-item') || item.data('selectable-item').selected) return null;
            this._selection.push(item);
            this.latestSelection = item;
            item.addClass('ui-selected ui-state-active');
            item.data('selectable-item').selected = true;
            if (item.filter(":last").length > 0) {
				if(this.options.callOnSelection)
				{
					this.options.callOnSelection(item.filter(":last"));
				}
            }
            if (triggerEvent) {
                this._trigger('select', triggerEvent, $.extend({
                    lasso: true
                }, this._uiHash(item)));
            }
            return item;
        ...
        _updateSelectionMouse: function (event) {
			...
			try{
            return $($.map(newlySelected, function (i) {
                return i[0];
            }));
			}catch(ex){
				return false;
        ...
        first: function (event) {
            this._select(event, this.items.filter(":first"));
        },
        last: function (event) {
            this._select(event, this.items.filter(":last"));
        },
		filter: function (event, character) {
			try{
				var matches = this.items.filter(":Contains('"+character+"')");
				if(matches && matches.length>0)
					this._select(event, matches.filter(":first"));
			}catch(ex){}
        },
        refresh: function (fromInside) {
            ...
            this.items.each(function () {
                var $this = $(this);
                var pos = $this.offset();
                if (self.currentFocus && self.currentFocus[0] != this) $this.attr('tabindex', -1);
        ...
        select: function (item) {
            ...
            // Ending the selection does a diff and then triggers appropriate events
            this._endSelection(event, $($.map(newlySelected, function (i) {
                return i ? i[0] : false;
            })));
        },
        ...
        _uiHash: function (items, specialKey) {
            var uiHash = {
                ...
                selection: $($.map(this._selection, function (i) {
                    return i[0];
                }))
            };
