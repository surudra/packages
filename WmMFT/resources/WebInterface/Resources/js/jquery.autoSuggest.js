...
(function ($) {
    ...
    function showList(data, list, text) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            {
            	curText = list[i].split("||");
            	curVal = curText[0];
            	curText = curText[0] + " : " + curText[1];
            }
            html += "<li data-value='" + curVal + "' " + width + ">" + curText.replace(regEx, "<mark>$1</mark>") + "</li>";
        }
        $(data.list).html(html).scrollTop(0);
    }
    ...
    function getOuterPosition(data) {
        var offset = $(data.ta).offset();
        return {
            top: offset.top + $(data.ta).height() + 8,
            left: offset.left
        };
    }
    ...
    function setSelected(dir, data) {
        ...
        var selection = $(data.list).find("li[data-selected='true']");
        if(selection && selection.length>0)
        {
        	if(!isScrolledToView(selection))
        	{
        		var parent = selection.parent();
        		parent.scrollTop($(selection).offsetRelativeTo(parent).top - 5);
        	}
        }
    }
    ...
    function isScrolledToView(elem){
        ...
        var elemTop = $(elem).offsetRelativeTo(parent).top + 8;
