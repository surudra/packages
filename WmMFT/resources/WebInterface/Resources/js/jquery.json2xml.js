...
(function($) {
	...
	var convertToXml = function(json, tagName, parentPath, depth) {
		...
		for (var key in json) {
			if (json.hasOwnProperty(key)) {
				var propertyPath = tagName;
				var propertyName = getPropertyName(parentPath, key);
				if(((propertyName - 0) == propertyName && propertyName.length > 0))
				{
					propertyName = tagName + "_subitem";
				}
				...
				if ($.inArray(propertyPath, settings.ignore) == -1) {
					if(!$.isFunction(json[key]))
					{
						...
						// primitive value property as attribute
							...
							// unless it's explicitly defined it should not be node
	...
	var createTextNode = function(name, text, parentDepth, suffix) {
		...
		var xmlNode = getIndent(parentDepth + 1) + '<' + name + '>'
					+ textPrefix + crushFTP.methods.htmlEncode(text) + textSuffix
					+ '</' + name + '>' + suffix;
		return xmlNode;
	};
})(jQuery);