...
Trigger to populate the table WM_TRACKCHANGES when there is an insert or update on OE_ORDER_HEADERS_ALL table and gives a call to WEB_TRANSACTION procedure to populate WM_TRACKCHANGES tables.

******************************************************************************************/

CREATE OR REPLACE TRIGGER &&apps_user..wm_order_headers_iu_trg
AFTER INSERT OR UPDATE ON OE_ORDER_HEADERS_ALL
FOR EACH ROW
WHEN (new.booked_flag = 'Y')
DECLARE
...
BEGIN
	...
	ELSIF UPDATING THEN
		IF :old.booked_date IS NULL then
			p_rec_wm_trackChange.transaction_id := :new.header_id;
    	 		p_rec_wm_trackChange.comments:='ORDER HEADER INSERT FOR '||:new.header_id;
     	 		p_rec_wm_trackChange.transaction_status := 1;
		ELSE
 	 		p_rec_wm_trackChange.transaction_id := :new.header_id;
    	 		p_rec_wm_trackChange.comments:='ORDER HEADER UPDATE FOR '||:new.header_id;
...
AFTER INSERT OR UPDATE OR DELETE ON OE_SALES_CREDITS
FOR EACH ROW
DECLARE
p_rec_wm_trackChange wm_trackchanges%ROWTYPE;
p_booked_flag        oe_order_headers_all.booked_flag%TYPE;
...
AFTER INSERT OR UPDATE OR DELETE ON OE_LOT_SERIAL_NUMBERS
FOR EACH ROW
DECLARE
p_rec_wm_trackChange wm_trackchanges%ROWTYPE;
c_transaction_type VARCHAR2(100):='SALESORDER';
	...
	IF p_booked_flag = 'Y' THEN
		...
		IF INSERTING THEN
	 		p_rec_wm_trackChange.transaction_id := p_header_id;
	    	 	p_rec_wm_trackChange.comments :=
			'LOT SERIAL INSERT FOR LINE OF ORDER HEADER '|| 
			p_header_id;
		ELSIF UPDATING THEN
	 		p_rec_wm_trackChange.transaction_id := p_header_id;
	    	 	p_rec_wm_trackChange.comments := 
			'LOT SERIAL UPDATE FOR LINE OF ORDER HEADER '|| 
			p_header_id;
		ELSIF DELETING THEN
	 		p_rec_wm_trackChange.transaction_id := p_header_id;
	    	 	p_rec_wm_trackChange.comments := 
...
AFTER INSERT OR UPDATE OR DELETE ON OE_PRICE_ADJUSTMENTS
FOR EACH ROW
DECLARE
p_rec_wm_trackChange wm_trackchanges%ROWTYPE;
p_booked_flag        oe_order_headers_all.booked_flag%TYPE;
...
BEGIN
	...
	IF p_booked_flag = 'Y' THEN
		...
		IF INSERTING THEN
	 		p_rec_wm_trackChange.transaction_id := :new.header_id;
	    	 	p_rec_wm_trackChange.comments :=
					'PRICE ADJUSTMENTS INSERT FOR ORDER '|| 
					:new.header_id;
		ELSIF UPDATING THEN
	 		p_rec_wm_trackChange.transaction_id := :new.header_id;
	    	 	p_rec_wm_trackChange.comments := 
					'PRICE ADJUSTMENTS UPDATE FOR ORDER '|| 
					:new.header_id;
		ELSIF DELETING THEN
	 		p_rec_wm_trackChange.transaction_id := :old.header_id;
	    	 	p_rec_wm_trackChange.comments := 
...
AFTER INSERT OR UPDATE OR DELETE ON OE_ORDER_PRICE_ATTRIBS
FOR EACH ROW
DECLARE
p_rec_wm_trackChange wm_trackchanges%ROWTYPE;
p_booked_flag        oe_order_headers_all.booked_flag%TYPE;
...
BEGIN
	...
	IF p_booked_flag = 'Y' THEN
		...
		IF INSERTING THEN
	 		p_rec_wm_trackChange.transaction_id := :new.header_id;
	    	 	p_rec_wm_trackChange.comments :=
				'ORDER PRICE ATTRIBUTES INSERT FOR ORDER '|| 
				:new.header_id;
		ELSIF UPDATING THEN
	 		p_rec_wm_trackChange.transaction_id := :new.header_id;
	    	 	p_rec_wm_trackChange.comments := 
				'ORDER PRICE ATTRIBUTES UPDATE FOR ORDER '|| 
				:new.header_id;
		ELSIF DELETING THEN
	 		p_rec_wm_trackChange.transaction_id := :old.header_id;
	    	 	p_rec_wm_trackChange.comments := 
...
AFTER INSERT OR DELETE ON OE_LINE_SETS
FOR EACH ROW
DECLARE
p_rec_wm_trackChange wm_trackchanges%ROWTYPE;
c_transaction_type VARCHAR2(100):='SALESORDER';
...
BEGIN
	...
	IF p_booked_flag = 'Y' THEN
		...
		IF INSERTING THEN
		 	p_rec_wm_trackChange.transaction_id := p_header_id;
    	 		p_rec_wm_trackChange.comments :=
				'ORDER LINE SET INSERT FOR ORDER HEADER '|| 
				p_header_id;
	     	 	p_rec_wm_trackChange.transaction_status := 0;
