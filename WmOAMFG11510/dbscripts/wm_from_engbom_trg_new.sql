/*  ***************************************************************************
    ...
    * Process Name:         Create Custom triggers for Engineering BOM Outbound 
    *                       in Application Schema  
    * Program Name:         wm_from_engbom_trg.sql
    * Version #:            1.0
...
*/
 ...
 prompt Program : wm_from_engbom_trg.sql
...
CREATE OR REPLACE TRIGGER &&apps_user..wm_eng_bom_bill_of_mtl_iud_trg INSTEAD OF
INSERT OR UPDATE OR DELETE ON BOM_BILL_OF_MATERIALS 
...
BEGIN
  IF NVL(:NEW.ASSEMBLY_TYPE,:OLD.ASSEMBLY_TYPE) = 2 THEN
...
CREATE OR REPLACE TRIGGER &&apps_user..wm_eng_mtl_item_rev_iud_trg 
AFTER INSERT OR UPDATE OR DELETE ON MTL_ITEM_REVISIONS_B
REFERENCING NEW AS NEW OLD AS OLD FOR EACH ROW
DECLARE
p_rec_wm_trackChange 	wm_trackchanges%ROWTYPE;
c_transaction_type 	VARCHAR2(100):='ENGBOM';
...
BEGIN
  ...
  IF (vBillSeqId IS NOT NULL) AND (vAssemblyType = 2) THEN
	-- assign data for parent key identifier
...
CREATE OR REPLACE TRIGGER &&apps_user..wm_eng_bom_inv_comps_iud_trg INSTEAD OF
INSERT OR UPDATE OR DELETE ON BOM_INVENTORY_COMPONENTS 
REFERENCING NEW AS NEW OLD AS OLD FOR EACH ROW
DECLARE
p_rec_wm_trackChange wm_trackchanges%ROWTYPE;
c_transaction_type VARCHAR2(100):='ENGBOM';
...
CREATE OR REPLACE TRIGGER &&apps_user..wm_eng_bom_sub_comps_iud_trg
...
CREATE OR REPLACE TRIGGER &&apps_user..wm_eng_bom_ref_desgs_iud_trg
