/*  ***************************************************************************
    ...
    *==========================================================================
     ...
     31-OCT-02	   Sudip Chaudhuri	   Removed AUTHID CURRENT_USER for 10.7 compatibility
     31-OCT-02	   Sudip Chaudhuri         Replaced SOURCE_LANG column with LANGUAGE for
     					   fnd_responsibility_tl table
     31-OCT-02	   Sudip Chaudhuri	   Changed EXECUTE IMMEDIATE call to dbms_sql for 7.3 compatibility
     19-NOV-02	   Koushik Chakraborty	   Changed fnd_responsibility_tl to fnd_responsibility
    ***************************************************************************
*/
...
CREATE OR REPLACE PACKAGE wm_track_changes_pkg AS
...
CREATE OR REPLACE PACKAGE Wm_Conc_Request_Pkg AS
...
BEGIN
	...
	--AND    language = userenv('LANG'); -- commented, not applicable for 10.7
...
CREATE OR REPLACE PACKAGE WM_UTILS_PKG AS
...
dbms_sql.parse(cur_hdl,v_select_stmt,dbms_sql.native);
dbms_sql.define_column(cur_hdl,1,v_seq_value);
v_count:=dbms_sql.EXECUTE(cur_hdl);
v_count:=dbms_sql.fetch_rows(cur_hdl);
dbms_sql.column_value(cur_hdl,1,v_seq_value);
dbms_sql.close_cursor(cur_hdl);

o_sequence_value:=v_seq_value;
