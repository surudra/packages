/*  ***************************************************************************
    ...
    * Process Name:         Create Custom Views for Vendor outbound in Application Schema  
    * Program Name:         wm_from_vendor_vw.sql
    * Version #:            1.0
...
*/
 ...
 prompt Program : wm_from_vendor_vw.sql
...
/*----------------------------------------------------------------------*/
--      Create View WM_PO_VENDORS_QRY_VW
/*----------------------------------------------------------------------*/

create or replace view wm_po_vendors_qry_vw as
...
WHERE
pov.employee_id = per.person_id(+)
AND  pov.parent_vendor_id=pov_parent.vendor_id(+)
...
/*----------------------------------------------------------------------*/
--      Create View WM_PO_VENDORS_VW
/*----------------------------------------------------------------------*/

CREATE OR REPLACE VIEW wm_po_vendors_vw AS
...
AND  wmtc.transaction_type = 'VENDOR'
AND  wmtc.transaction_status <=2
/

/*----------------------------------------------------------------------*/
--      Create View WM_PO_VENDOR_SITES_ALL_VW
/*----------------------------------------------------------------------*/

CREATE OR REPLACE VIEW WM_PO_VENDOR_SITES_ALL_VW
AS 
SELECT  
povs.vendor_id,  
povs.vendor_site_id,  
...
/*----------------------------------------------------------------------*/
--      Create View WM_PO_VENDOR_CONTACTS_VW
/*----------------------------------------------------------------------*/

CREATE OR REPLACE VIEW WM_PO_VENDOR_CONTACTS_VW AS 
SELECT 
povc.vendor_site_id,  
...
/*----------------------------------------------------------------------*/
--      Create View WM_BANK_ACC_USE_VEND_SITES_VW
/*----------------------------------------------------------------------*/


CREATE OR REPLACE VIEW wm_bank_acc_use_vend_sites_vw 
( row_id, 
vendor_id, 
vendor_site_id, 
bank_account_name, 
...
WHERE  
  ...
  abau.org_id = aba.org_id(+)    AND
  aba.org_id = pvs.org_id(+)    AND
  abau.org_id=org.organization_id(+)
/

/*----------------------------------------------------------------------*/
--      Create View WM_BANK_ACCOUNT_USES_VENDOR_VW
/*----------------------------------------------------------------------*/


CREATE OR REPLACE VIEW wm_bank_account_uses_vendor_vw 
( row_id, 
vendor_id, 
vendor_site_id, 
bank_account_name, 
...
WHERE  
  ...
  abau.vendor_id = pv.vendor_id(+)    AND
  abau.org_id = aba.org_id(+)    AND
  abau.org_id=org.organization_id(+)    AND
  abau.vendor_site_id IS NULL
/
