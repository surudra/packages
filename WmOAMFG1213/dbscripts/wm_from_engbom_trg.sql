...
CREATE OR REPLACE TRIGGER &&apps_user..wm_eng_bom_bill_of_mtl_iud_trg
AFTER DELETE ON bom_structures_b
...
AFTER DELETE ON MTL_ITEM_REVISIONS_B
REFERENCING NEW AS NEW OLD AS OLD FOR EACH ROW
DECLARE
p_rec_wm_trackChange 	wm_trackchanges%ROWTYPE;
c_transaction_type 	VARCHAR2(100):='ENGBOM';
...
CREATE OR REPLACE TRIGGER &&apps_user..wm_eng_bom_inv_comps_iud_trg
AFTER DELETE ON bom_components_b
REFERENCING NEW AS NEW OLD AS OLD FOR EACH ROW
DECLARE
p_rec_wm_trackChange wm_trackchanges%ROWTYPE;
c_transaction_type VARCHAR2(100):='ENGBOM';
...
AFTER DELETE ON BOM_SUBSTITUTE_COMPONENTS
REFERENCING NEW AS NEW OLD AS OLD FOR EACH ROW
DECLARE
p_rec_wm_trackChange 	wm_trackchanges%ROWTYPE;
c_transaction_type 	VARCHAR2(100):='ENGBOM';
...
AFTER DELETE ON BOM_REFERENCE_DESIGNATORS
REFERENCING NEW AS NEW OLD AS OLD FOR EACH ROW
DECLARE
p_rec_wm_trackChange wm_trackchanges%ROWTYPE;
c_transaction_type VARCHAR2(100):='ENGBOM';
