...
CREATE OR REPLACE PACKAGE WM_MFGBOM_FETCH_PKG AUTHID CURRENT_USER AS

PROCEDURE wm_fetch_mfgbom(record_count OUT NUMBER);

END WM_MFGBOM_FETCH_PKG;
/

CREATE OR REPLACE PACKAGE BODY WM_MFGBOM_FETCH_PKG AS
 
PROCEDURE wm_fetch_mfgbom(record_count OUT NUMBER) IS
...
BEGIN
   ...
   IF (V_BILL_SEQUENCE_ID IS NOT NULL) AND (V_ASSEMBLY_TYPE = 1) THEN
	  p_rec_wm_trackChange.transaction_type := c_transaction_type;
