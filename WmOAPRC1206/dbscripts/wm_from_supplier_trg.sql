...
or delete on AP_SUPPLIERS table and gives a call to WEB_TRANSACTION procedure to populate  
WM_TRACKCHANGES tables.

******************************************************************************************/

CREATE OR REPLACE TRIGGER &&apps_user..wm_po_supplier_iu_trg
AFTER INSERT OR UPDATE ON AP_SUPPLIERS
...
Trigger to populate the table WM_TRACKCHANGES when there is an insert or update
or delete on PO_VENDORS_SITES_ALL table and gives a call to WEB_TRANSACTION procedure to populate  
WM_TRACKCHANGES tables.
...
AFTER INSERT OR UPDATE OR DELETE ON AP_SUPPLIER_CONTACTS
REFERENCING NEW AS NEW OLD AS OLD FOR EACH ROW
DECLARE
p_rec_wm_trackChange wm_trackchanges%ROWTYPE;
v_transaction_type VARCHAR2(100):='SUPPLIER';
...
Trigger to populate the table WM_TRACKCHANGES when there is an insert or update
or delete on AP_BANK_ACCOUNT_USES_ALL table and gives a call to WEB_TRANSACTION procedure to populate  
WM_TRACKCHANGES tables.
